#!/usr/bin/env python3
"""
meow OAuth Wizard — OAuth-only Cloudflare Worker deployer.
"""
...
def safe_name(v, fallback="meow-panel"):
    c = "".join(ch.lower() if ch.isalnum() else "-" for ch in (v or "").strip())
    while "--" in c: c = c.replace("--", "-")
    return c.strip("-") or fallback

_WORDS_A = ["sunny","meow","swift","neon","atlas","orbit","pixel","rocket","falcon","crystal","rainbow","mango","coral","luna","pearl","turbo"]
...
def rand_name(max_len=55):
    for _ in range(50):
        ...
        s = safe_name(s, "meow-panel")[:max_len].strip("-")
        if s: return s
    return f"meow-panel-{secrets.token_hex(4)}"[:max_len]

def suggest():
    w = rand_name(55)
    kv = safe_name(f"{w}-{secrets.choice(_STORE_WORDS)}", "meow-panel-vault")[:60].strip("-") or f"meow-{secrets.token_hex(4)}-vault"[:60]
    d1 = safe_name(f"{w}-db", "meow-panel-db")[:32].strip("-") or f"meow-db-{secrets.token_hex(4)}"[:32]
    ...
    def do_GET(self):
        ...
        if p.path == "/oauth/callback":
            ...
            html = (
                ...
                '<h1 style="font-size:1.4rem;font-weight:800;margin:0 0 14px">meow Wizard</h1>'
...
class CFClient:
    ...
    def req(self, method, path, *, json_body=None, data=None, ctype=None, extra_headers=None, accept_404=False):
        h = {"Authorization": f"Bearer {self.token}", "Accept": "application/json", "User-Agent": "meow-wizard/3.0"}
...
def get_subdomain(cf, aid):
    ...
    des = safe_name(f"meow-{secrets.token_hex(4)}", "meow-panel")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def unauth(self):
        ...
        page = (
            ...
            '<h1 style="font-size:1.3rem;font-weight:800;margin:0 0 12px">meow Wizard</h1>'
...
def main():
    ...
    port = int(os.environ.get("meow_PORT", "8000"))
    httpd = ThreadingHTTPServer((host, port), Handler)
    url = f"http://{host}:{port}/?token={LOCAL_TOKEN}"
    print()
    print("=" * 54)
    print("  meow OAuth Wizard")
